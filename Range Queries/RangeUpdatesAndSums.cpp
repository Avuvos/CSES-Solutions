...
//we set value, then we increment
struct Update {
    ll set_value, increment;
    Update() {
        set_value = -1;
        increment = 0;
    }

    Update(ll set_value, ll increment) {
...
struct Lsegtree {
    ...
    T apply(T curr, U upd, ll tl, ll tr) {
        T ans;
        if (upd.set_value != -1) { //modify
            ans = (tr - tl + 1) * (upd.set_value + upd.increment);
        } else { //increment
            ans = curr + (tr - tl + 1) * upd.increment;
        }
        return ans;
    }
    ...
    U combineUpdate(U old_update, U new_update, ll tl, ll tr) {
        U ans;
        if (new_update.set_value != -1) { //modify
            ans.set_value = new_update.set_value;
            ans.increment = new_update.increment;
        } else { //increment
            ans.set_value = old_update.set_value;
            ans.increment = old_update.increment + new_update.increment;
        }
        return ans;
    }
...
void solve() {
    ...
    for (int i = 0; i < q; i++) {
        ...
        if (op == 1) {
            cin >> x;
            Update current_update(-1, x);
            st.update(l, r, current_update);
        } else if (op == 2) {
            cin >> x;
            Update current_update(x, 0);
            st.update(l, r, current_update);
        } else if (op == 3) {
            cout << st.query(l, r) << '\n';
        } else {
